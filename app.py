...
def fetch_article_route():
    ...
    try:
        ...
        return jsonify({"title": metadata.title, "content": article})
...
def convert_article_to_audio(job):
    ...
    try:
        ...
        if os.path.exists(article_output_dir):
            ...
            log_message(
                job_id, f"[INFO] Clearing existing folder: {article_output_dir}"
            )
        ...
        for i, chunk in enumerate(chunks):
            output_file = os.path.join(
                article_output_dir, f"{safe_title}_{i + 1:03d}.wav"
            )
            log_message(
                job_id, f"[CHUNK {i + 1}/{len(chunks)}] Generating audio..."
            )
        ...
        # Combine
        log_message(
            job_id, f"[INFO] Combining {len(chunks)} chunks into MP3..."
        )
        ...
        # keep progress in tracker for later viewing
...
def queue_status():
    ...
    for idx, job in enumerate(list(job_queue.queue), start=2):
        queued.append({"job_id": job["job_id"], "title": job["title"], "position": idx})

    with completed_jobs_lock:
        completed = [j.copy() for j in completed_jobs]

    return jsonify({"current": curr, "queued": queued, "completed": completed})
...
def convert():
    ...
    if job_queue.full():
        return (
            jsonify({"error": "Queue is full (max 10 jobs). Try again later."}),
            429,
        )
    ...
    try:
        ...
        return jsonify(
            {"job_id": job_id, "position": job_queue.qsize() + (1 if current_job else 0)}
        )
    except Exception as e:
        return jsonify({"error": str(e)}), 500
